...
def copy_data(pk_maps, source_model: models.Model, target_model: models.Model):
    ...
    fields = [field.name for field in source_model._meta.get_fields() if not (field.many_to_one or field.many_to_many or field.one_to_many)]
...
def migrate_from_blog_to_stories(apps, schema_editor):
    ...
    if required_migration not in recorder.applied_migrations():
        raise RuntimeError(
            "First run 'python manage migrate djangocms_blog'."
        )
    ...
    m2m_models = [
        ...
        ('djangocms_blog', 'AuthorEntriesPlugin', 'AuthorEntriesPlugin_authors+', 'djangocms_stories', 'AuthorEntriesPlugin', 'authors'),
    ]
